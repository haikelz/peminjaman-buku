...
export default function Sidebar({ sidebarOpen, setSidebarOpen }: SidebarProps) {
  ...
  useEffect(() => {
    function clickHandler({ target }: MouseEvent) {
      ...
      if (
        !sidebarOpen ||
        sidebarRef.current.contains(target as any) ||
  ...
  return (
    <aside
      ...
      ref={sidebarRef}
      className={tw(
        "absolute left-0 top-0 z-9999 flex h-screen w-72.5 flex-col overflow-y-hidden", 
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5">
        <Link href="/">
          <h1 className="font-bold text-2xl text-white">Peminjaman Buku</h1>
        </Link>
        <button
          ref={triggerRef}
