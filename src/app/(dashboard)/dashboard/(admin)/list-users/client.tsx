...
export default function ListUsersClient({ listUsers }: { listUsers: ListUsersProps[] }) {
  ...
  return (
    <>
      {listUsers.length ? <CSVDownloader filename="list_users" data={listUsers} /> : null}
      <div className="flex flex-col gap-10 mt-5">
        {listUsers.length ? (
            ...
            tableData={currentBooks.map((item, key) => (
              <tr key={key}>
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <p className="text-black dark:text-white font-medium">{item.judul_buku}</p>
                </td>
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <p
                    className={tw(
          ...
          <h3 className="text-center font-bold text-2xl">Belum ada user yang meminjam buku!</h3>
        )}
      </div>
